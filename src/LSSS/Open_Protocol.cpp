 ...
 */
void Open_Protocol::RunOpenCheck(Player &P, const string &auxfiles, int connection, bool verbose)
{
  if (Share::SD.type == Full)
    {
      ...
      if (verbose)
        {
          ...
          for (unsigned int i= 0; i < auxfiles.size(); i++)
            {
              printf("%02x", (uint8_t) auxfiles.c_str()[i]);
            }
          printf("\n");
        }
      P.send_all(auxfiles, connection, verbose);
      for (unsigned int i= 0; i < P.nplayers(); i++)
        {
          if (i != P.whoami())
            {

               ...
               unsigned int number_FHE_threads, int verbose)
{
  ...
  cout << "Adding signal handler" << endl;
  shutdown_handler= [&](int sig) {
    ...
    try
      ...
      {
        cout << "Caught exception in Close_Connections" << endl;
      }
    cout << "All connections now closed" << endl;
    exit(sig);
  };

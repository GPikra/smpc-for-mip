  ...
  do
    {
      ...
      while (fl == 1 || errno == EINPROGRESS)
        {
          cout << "Trying to connect to " << hostname << " on port " << port
               << endl;
          fl= connect(sd, (struct sockaddr *) &addr, sizeof(struct sockaddr));
          cout << "Connect returned " << fl << " with errno " << errno << endl;
        }
                     ...
                     const SystemData &SD, int verbose)
{
  ...
  printf("Opening listener: %d, %d\n", portnum[me], 2 * SD.n * nthreads);
  ssocket= OpenListener(portnum[me], 2 * SD.n * nthreads);
  cout << "Opened Listener" << endl;
                  ...
                  for (unsigned int k= 0; k < 2; k++)
                    {
                      ...
                      cout << "S: Connection: " << inet_ntoa(addr.sin_addr) << ":"
                      ...
                      cout << "S: Player " << me << " connected to " << p << " on connection " << c
                           << " for thread " << t << endl;
                    }
                }
            }
              ...
              for (unsigned int j= 0; j < nthreads; j++)
                {
                  for (unsigned int k= 0; k < 2; k++)
                    {
                      cout << "C: Player " << me << " connecting to player " << i << " on connection "
                           << k << " at address " << SD.IP[i] << " for thread " << j << " on port "
                           << portnum[i] << endl;
                      csocket[j][i][k]= OpenConnection(SD.IP[i], portnum[i]);
                      cout << "C: Player " << me << " connected to " << i << " on connection " << k
                           << " for thread " << j << endl;

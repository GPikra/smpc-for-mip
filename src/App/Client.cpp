...
sedp::Client::Client(string id, unsigned int max_players, string dataset) : client_id{id}, max_players{max_players}, dataset_file_path{dataset}, dataset_size{0}
{
...
void sedp::Client::init()
{
  ...
  cout << "HERE IS MY DATA (updated)" << endl;
...
void sedp::Client::Init_SSL_CTX(SSL_CTX *&ctx)
{
  ...
  string str_crt= "Cert-Store/Client" + client_id + ".crt";
...
void sedp::Client::handshake(int player_id)
{
  ...
  send_str_to(players.at(player_id), client_id);

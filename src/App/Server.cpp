...
SSL *sedp::Server::accept_single_client()
{
  ...
  cout << "Accepted Connection: " << inet_ntoa(addr.sin_addr) << " " << ntohs(addr.sin_port) << endl;
  return ssl;
}
...
void sedp::Server::handshake(SSL *ssl)
{
  ...
  string client_id= receive_str_from(ssl);
  int dataset_size= receive_int_from(ssl);
  lock_guard<mutex> g{mtx};
  pair<string, int> c(client_id, dataset_size);
  clients.insert(pair<pair<string, int>, SSL *>(c, ssl));
...
void sedp::Server::handle_clients()
{
  ...
  map<pair<string, int>, SSL *>::iterator itr;
  int start= 0;
  int end= 0;
  ...
  for (itr= clients.begin(); itr != clients.end(); ++itr)
    {
      ...
      int dataset_size= itr->first.second;
      SSL *ssl= itr->second;

      end+= dataset_size;

      responses.push_back(async(
          launch::async, [this](SSL *ssl, int dataset_size, int start, int end) -> vector<gfp> // return vector<int>
          {
            vector<gfp> v;
